...
管理区提供独立的 `/resource-pools` 页面，展示名称、所属供应商、故障域、模型、渠道数、共享容量和启用状态。支持名称/ID 搜索，供应商、状态、模型和故障域筛选及分页；筛选与页码保存在 URL 中。供应商列表和详情的“查看资源池”跳转到该页面并自动筛选供应商，单选供应商时新建会自动带入归属。

“添加资源池”及编辑均在抽屉中完成：基础信息（供应商、名称、故障域、验收报告）→ 共享容量 → 模型信息（选择平台模型、多选渠道、填写验收规格与分模型限制）→ 保存资源池。创建时可选择供应商，切换归属会清空草稿中的渠道选择；已创建资源池的供应商不可修改。相同资源池可以配置多个模型；增加渠道不会增加资源池总并发、RPM、TPM。

列表提供编辑/查看、启停、删除。启停只 PATCH 当前记录的 `enabled`，删除时明确提示解除关联并保留渠道本身。模型声明预览位于资源池编辑抽屉。供应商抽屉仅管理供应商信息，调度策略仍由供应商页面进入。

复用现有只读概览中的完整资源集合（资源池上限仍为 128）和单记录写入接口，无新增表或数据库迁移。全量读取仅用于展示与选项，保存始终按记录执行。前端删除已无入口的独立绑定类型、表单及路径分支，后端兼容绑定 API 保留。
