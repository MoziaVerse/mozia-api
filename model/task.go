...
type TaskBillingContext struct {
	...
	TaskTokenPrice        *taskbilling.TokenPriceResult `json:"task_token_price,omitempty"`          // 参数化 token 单价快照
	TaskTokenQuotaPerUnit float64                       `json:"task_token_quota_per_unit,omitempty"` // 提交时 USD 到额度换算快照
...
func GetTimedOutUnfinishedTasks(cutoffUnix int64, limit int) []*Task {
	...
	err := DB.Where("progress != ?", "100%").
		...
		// Ark can queue for up to 48 hours; only its terminal status authorizes a refund.
		Where("platform IS NULL OR platform != ?", constant.TaskPlatformVolcengineVideo).
